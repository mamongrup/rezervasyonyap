//// Acente faturaları — `commission_accrual_lines` üzerinden (190_agency_invoices).
...
/// `commission_accrual_lines.created_at` — persisted ile aynı aralık ($2, $3) + org $1.
fn cal_date_filter() -> String {
  "cal.agency_organization_id = $1::uuid and cal.agency_invoice_id is null and cal.status = 'final' and cal.created_at >= (case when trim($2) = '' then (current_date - interval '30 days')::timestamptz else trim($2)::date::timestamptz end) and cal.created_at < (case when trim($3) = '' then (current_date + interval '1 day')::timestamptz else (trim($3)::date + interval '1 day')::timestamptz end)"
}

/// INSERT…SELECT için: $1 = invoice_id, $2 = org, $3/$4 = tarih, $5 = para birimi.
fn cal_date_filter_for_insert_lines() -> String {
  "cal.agency_organization_id = $2::uuid and cal.agency_invoice_id is null and cal.status = 'final' and cal.created_at >= (case when trim($3) = '' then (current_date - interval '30 days')::timestamptz else trim($3)::date::timestamptz end) and cal.created_at < (case when trim($4) = '' then (current_date + interval '1 day')::timestamptz else (trim($4)::date + interval '1 day')::timestamptz end) and cal.currency_code = $5"
}
...
/// GET — son 50 fatura.
pub fn list_response(conn: pog.Connection, agency_org_id: String) -> Response {
  case
    pog.query(
      "select ai.id::text, ai.period_from::text, ai.period_to::text, ai.currency_code::text, ai.gross_total::text, ai.commission_total::text, ai.line_count::text, ai.status::text, ai.invoice_number, coalesce(ai.notes, '')::text, to_char(ai.created_at, 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"') from agency_invoices ai where ai.agency_organization_id = $1::uuid order by ai.created_at desc limit 50",
    )
...
/// GET — tek fatura + satır kalemleri.
pub fn get_invoice_response(
  ...
  let head_sql =
    "select ai.id::text, ai.period_from::text, ai.period_to::text, ai.currency_code::text, ai.gross_total::text, ai.commission_total::text, ai.line_count::text, ai.status::text, ai.invoice_number, coalesce(ai.notes, '')::text, to_char(ai.created_at, 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"') from agency_invoices ai where ai.id = $1::uuid and ai.agency_organization_id = $2::uuid"
  case
    pog.query(head_sql)
    ...
    Ok(hret) ->
      case hret.rows {
        ...
        [row] -> {
          ...
          let lines_sql =
            "select ail.id::text, ail.commission_accrual_line_id::text, ail.reservation_id::text, ail.public_code, ail.gross_amount::text, ail.commission_amount::text, ail.currency_code::text from agency_invoice_lines ail where ail.invoice_id = $1::uuid order by ail.public_code"
          case
            pog.query(lines_sql)
...
pub fn preview_response(
  ...
  case resolve_currency(conn, agency_org_id, from_q, to_q, currency_opt) {
    Error(r) -> r
    Ok(cur) -> {
      let sql =
        string.concat([
...
/// Fatura oluşturur: başlık + satırlar (INSERT…SELECT) + tahakkuk bağlantısı.
                      ...
                      case ins_ret.rows {
                        [inv_id] -> {
                            ...
                            |> pog.execute(conn)
                          {
                            ...
                            Ok(_) -> {
                              let upd_cal =
                                "update commission_accrual_lines cal set agency_invoice_id = $1::uuid where cal.id in (select ail.commission_accrual_line_id from agency_invoice_lines ail where ail.invoice_id = $1::uuid)"
                              case
                                pog.query(upd_cal)
                                |> pog.parameter(pog.text(inv_id))
                                |> pog.execute(conn)
                              {
                                ...
                                Ok(_) -> {
                                  let upd_num =
                                    "update agency_invoices set invoice_number = 'AGC-' || to_char(now(), 'YYYYMMDD') || '-' || upper(substr(replace(id::text,'-',''), 1, 8)) where id = $1::uuid and invoice_number = 'pending' returning invoice_number"
                                  case
                                    pog.query(upd_num)
                                    |> pog.parameter(pog.text(inv_id))
                                    ...
                                    |> pog.execute(conn)
                                  {
                                    ...
                                    Ok(nr) ->
                                      case nr.rows {
                                        [inv_no] -> {
                                          invoice_notify.agency_invoice_created(
                                            notify,
                                            agency_org_id,
                                            inv_id,
                                            inv_no,
...
/// İptal: tahakkuk bağlarını kaldırır, başlık `cancelled` olur (satır kayıtları kalır).
pub fn cancel_response(
  ...
  case
    pog.query(
      "select status::text, invoice_number::text from agency_invoices where id = $1::uuid and agency_organization_id = $2::uuid",
    )
    |> pog.parameter(pog.text(invoice_id))
    |> pog.parameter(pog.text(agency_org_id))
        ...
        [#(st, inv_no)] ->
          case st {
            ...
            "issued" ->
              case
                pog.query(
                  "update commission_accrual_lines set agency_invoice_id = null where agency_invoice_id = $1::uuid",
                )
                |> pog.parameter(pog.text(invoice_id))
                |> pog.execute(conn)
              {
                ...
                Ok(_) ->
                  case
                    pog.query(
                      "update agency_invoices set status = 'cancelled' where id = $1::uuid and agency_organization_id = $2::uuid and status = 'issued'",
                    )
                    |> pog.parameter(pog.text(invoice_id))
                    |> pog.parameter(pog.text(agency_org_id))
                    |> pog.execute(conn)
                  {
                    ...
                    Ok(_) -> {
                      invoice_notify.agency_invoice_cancelled(
                        notify,
                        agency_org_id,
                        invoice_id,
                        inv_no,
...
/// Yalnızca `issued` faturalarda not günceller.
pub fn patch_notes_response(
  ...
  case
    pog.query(
      "update agency_invoices set notes = case when trim($3::text) = '' then null else trim($3::text) end where id = $1::uuid and agency_organization_id = $2::uuid and status = 'issued' returning id::text",
    )
    |> pog.parameter(pog.text(invoice_id))
    |> pog.parameter(pog.text(agency_org_id))
