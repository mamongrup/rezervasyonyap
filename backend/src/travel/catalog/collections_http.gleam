...
fn search_listings_impl(
  ...
  let sql =
    ...
    <> "coalesce(nullif(trim(both ', ' from concat_ws(', ', nullif(trim(regexp_replace(lm.meta->>'city', '\\s+[Yy]at\\s+[Kk]iralama', '', 'gi')), ''), nullif(trim(regexp_replace(lm.meta->>'district_label', '\\s+[Yy]at\\s+[Kk]iralama', '', 'gi')), ''), (case when trim(coalesce(lm.meta->>'province_city', '')) ~ '/' then nullif(trim(substring(trim(lm.meta->>'province_city') from '[^/]+$')), '') else nullif(trim(regexp_replace(lm.meta->>'province_city', '\\s+[Yy]at\\s+[Kk]iralama', '', 'gi')), '') end))), ''), nullif(trim(regexp_replace(l.location_name, '\\s+[Yy]at\\s+[Kk]iralama', '', 'gi')), ''), nullif(trim(lm.meta->>'region_display'), ''), nullif(trim(lm.meta->>'address'), ''), ''), "
