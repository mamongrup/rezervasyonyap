              ...
              case parse_iso_date_ymd(from_d), parse_iso_date_ymd(to_d) {
                Ok(from_date), Ok(to_date) ->
                  case
                    pog.query(
                      "select day::text, "
                      <> "(coalesce(am_available, is_available, false) or coalesce(pm_available, is_available, false) or is_available is true), "
                      <> "coalesce(price_override::text,''), coalesce(am_available, is_available, false), coalesce(pm_available, is_available, false), coalesce(day_status::text,'') "
                      <> "from listing_availability_calendar where listing_id = $1::uuid and day >= $2::date and day <= $3::date order by day",
                    )
                    |> pog.parameter(pog.text(listing_id))
                    |> pog.parameter(pog.calendar_date(from_date))
                    |> pog.parameter(pog.calendar_date(to_date))
...
fn listing_basics_manage_row() -> decode.Decoder(
  ...
  use locked <- decode.field(18, decode.bool)
  decode.success(#(
    status,
    msn,
    cfa,
              ...
              case json.parse(body, patch_basics_decoder()) {
                ...
                Ok(p) -> {
                  ...
                  case list.contains(valid_statuses, status_t) {
                    ...
                    True ->
                      // Kilitli ilan kontrolü: draft/archived'a geçiş engellensin
                      case
                        list.contains(["draft", "archived"], status_t)
                        && p.is_locked != option.Some(False)
                      {
                        True ->
                          case
                            pog.query(
                              "select coalesce(is_locked, false) from listings where id = $1::uuid",
                            )
                            |> pog.parameter(pog.text(listing_id))
                            |> pog.returning(pg_bool_row())
                            |> db_exec.execute(ctx.db)
                          {
                            ...
                            Ok(lock_ret) ->
                              case lock_ret.rows {
                                [True] -> json_err(409, "listing_is_locked")
                                _ -> do_patch_listing_basics(ctx.db, listing_id, org_id, p, status_t)
                              }
                          }
                        False -> do_patch_listing_basics(ctx.db, listing_id, org_id, p, status_t)
                      }
                  }
                }
              }
      ...
      case parse_iso_date_ymd(from_d), parse_iso_date_ymd(to_d) {
        Ok(from_date), Ok(to_date) ->
          case
            pog.query(
              "select c.day::text, "
              <> "(coalesce(c.am_available, c.is_available, false) or coalesce(c.pm_available, c.is_available, false) or c.is_available is true), "
              <> "coalesce(c.price_override::text,''), coalesce(c.am_available, c.is_available, false), coalesce(c.pm_available, c.is_available, false), coalesce(c.day_status::text,'') "
...
pub fn get_public_listing_vitrine(
  ...
  case
    pog.query(
      ...
      <> "coalesce(nullif(trim((select case "
      <> "when lower(trim(coalesce(la.value_json->>'district_label', ''))) in (lower(trim(coalesce(la.value_json->>'city', ''))), lower(trim(coalesce(la.value_json->>'province_city', '')))) then '' "
      <> "else coalesce(la.value_json->>'district_label', '') end "
