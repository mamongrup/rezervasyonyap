          ...
          case json.parse(body, gen_prov_decoder()) {
            ...
            Ok(#(cn, cid_opt)) ->
              case cn == "" {
                ...
                False ->
                  case
                    region_provinces_sync.generate_and_insert_provinces(
                      ctx,
                      cn,
                      cid_opt,
                    )
          ...
          case json.parse(body, gen_region_id_decoder()) {
            ...
            Ok(rid) ->
              case rid == "" {
                ...
                False ->
                  case
                    region_hierarchy_sync.generate_and_insert_districts(
                      ctx,
                      rid,
                    )
                  {
                    ...
                    Ok(out) ->
                      wisp.json_response(
                        gen_outcome_json(out) |> json.to_string,
                        200,
                      )
                  }
              }
          ...
          case json.parse(body, gen_district_id_decoder()) {
            ...
            Ok(did) ->
              case did == "" {
                ...
                False ->
                  case
                    region_hierarchy_sync.generate_and_insert_destinations(
                      ctx,
                      did,
                    )
                  {
            ...
            Ok(#(rid, et)) ->
              case string.trim(rid) == "" {
                ...
                False -> {
                  ...
                  case
                    ops_agent_enqueue.enqueue_ops_agent_job(ctx, rid, ev, True)
                  {
                    Error(e) ->
                      case e {
...
fn patch_key_decoder() -> decode.Decoder(
  #(String, Bool, String, Bool, Bool, Bool, Bool),
) {
...
fn complete_body_decoder() -> decode.Decoder(
  #(String, String, Float, Int, String, String),
) {
    ...
    decode.field("user", decode.string, fn(usr) {
      decode.optional_field("temperature", 0.3, decode.float, fn(temp) {
        decode.optional_field("timeout_ms", 90_000, decode.int, fn(to) {
          decode.optional_field("image_mime", "", decode.string, fn(mime) {
            decode.optional_field("image_base64", "", decode.string, fn(image) {
              decode.success(#(sys, usr, temp, to, mime, image))
            })
          })
        })
      })
          ...
          case string.trim(body) == "" {
            ...
            False ->
              case json.parse(body, complete_body_decoder()) {
                Error(_) -> json_err(400, "invalid_json")
                Ok(#(sys, usr, temp, timeout_ms, image_mime, image_base64)) -> {
                  let system = string.trim(sys)
                  let user = string.trim(usr)
                  let image = string.trim(image_base64)
                  let mime = string.trim(image_mime)
                  case
                    system == ""
                    || user == ""
                    || string.length(image) > 3_000_000
                  {
                      ...
                      let completion = case image == "" {
                        ...
                        False ->
                            ...
                            || mime == "image/webp"
                          {
                            ...
                            False -> Error("unsupported_image_mime")
                          }
                      }
                      case completion {
