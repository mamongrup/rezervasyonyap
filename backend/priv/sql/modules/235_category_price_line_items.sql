...
CREATE TABLE category_price_line_items (
...
CREATE INDEX idx_cpli_org_cat ON category_price_line_items (organization_id, category_code, sort_order);

CREATE TABLE category_price_line_item_translations (
...
CREATE TABLE listing_price_line_selections (
  listing_id UUID NOT NULL REFERENCES listings (id) ON DELETE CASCADE,
  item_id    UUID NOT NULL REFERENCES category_price_line_items (id) ON DELETE CASCADE,
  PRIMARY KEY (listing_id, item_id)
);

CREATE INDEX idx_lpls_item ON listing_price_line_selections (item_id);
