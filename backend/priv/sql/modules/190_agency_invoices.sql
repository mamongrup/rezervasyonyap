...
CREATE INDEX idx_agency_invoices_org ON agency_invoices (agency_organization_id, created_at DESC);
...
CREATE INDEX idx_agency_invoice_lines_invoice ON agency_invoice_lines (invoice_id);

ALTER TABLE commission_accrual_lines
  ADD COLUMN agency_invoice_id UUID REFERENCES agency_invoices (id) ON DELETE SET NULL;

CREATE INDEX idx_commission_accrual_lines_agency_invoice
