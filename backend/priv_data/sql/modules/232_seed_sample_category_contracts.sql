-- MODÜL: Emsal kategori sözleşmeleri (Airbnb/Vira Vira/OTA/tur platformları tarzı örnek metinler)
-- Önkoşul: 205_contract_scopes_general_sales.sql
-- NOT: Hukuki danışmanlık değildir; yönetim panelinden düzenlenmelidir.
-- Çeviriler: tr, en, de, ru, fr, zh (site dilleri ile uyumlu)
...
SELECT NULL, NULL, 'platform_emsal_genel_v1', 1, TRUE, 0, 'general'
WHERE NOT EXISTS (
  SELECT 1 FROM category_contracts c
  WHERE c.organization_id IS NULL AND c.contract_scope = 'general' AND c.code = 'platform_emsal_genel_v1'
);
...
WHERE cc.organization_id IS NULL AND cc.contract_scope = 'general' AND cc.code = 'platform_emsal_genel_v1'
  AND lower(loc.code) IN ('tr', 'en', 'de', 'ru', 'fr', 'zh')
...
INSERT INTO category_contracts (category_id, organization_id, code, version, is_active, sort_order, contract_scope)
SELECT NULL, NULL, 'platform_emsal_satis_v1', 1, TRUE, 0, 'sales'
WHERE NOT EXISTS (
  SELECT 1 FROM category_contracts c
  WHERE c.organization_id IS NULL AND c.contract_scope = 'sales' AND c.code = 'platform_emsal_satis_v1'
);
...
INSERT INTO category_contracts (category_id, organization_id, code, version, is_active, sort_order, contract_scope)
SELECT pc.id, NULL, 'emsal_airbnb_tarzi_v1', 1, TRUE, 5, 'category'
FROM product_categories pc
WHERE pc.code = 'holiday_home'
  AND NOT EXISTS (
    SELECT 1 FROM category_contracts x
    WHERE x.category_id = pc.id AND x.organization_id IS NULL AND x.code = 'emsal_airbnb_tarzi_v1'
  );
...
INSERT INTO category_contracts (category_id, organization_id, code, version, is_active, sort_order, contract_scope)
SELECT pc.id, NULL, 'emsal_vira_tarzi_v1', 1, TRUE, 5, 'category'
FROM product_categories pc
WHERE pc.code = 'yacht_charter'
  AND NOT EXISTS (
    SELECT 1 FROM category_contracts x
    WHERE x.category_id = pc.id AND x.organization_id IS NULL AND x.code = 'emsal_vira_tarzi_v1'
  );
...
INSERT INTO category_contracts (category_id, organization_id, code, version, is_active, sort_order, contract_scope)
SELECT pc.id, NULL, 'emsal_ota_otel_v1', 1, TRUE, 5, 'category'
FROM product_categories pc
WHERE pc.code = 'hotel'
  AND NOT EXISTS (
    SELECT 1 FROM category_contracts x
    WHERE x.category_id = pc.id AND x.organization_id IS NULL AND x.code = 'emsal_ota_otel_v1'
  );
...
INSERT INTO category_contracts (category_id, organization_id, code, version, is_active, sort_order, contract_scope)
SELECT pc.id, NULL, 'emsal_gezinomi_tarzi_v1', 1, TRUE, 5, 'category'
FROM product_categories pc
WHERE pc.code = 'tour'
  AND NOT EXISTS (
    SELECT 1 FROM category_contracts x
    WHERE x.category_id = pc.id AND x.organization_id IS NULL AND x.code = 'emsal_gezinomi_tarzi_v1'
  );
...
INSERT INTO category_contracts (category_id, organization_id, code, version, is_active, sort_order, contract_scope)
SELECT pc.id, NULL, 'emsal_ucus_bilet_v1', 1, TRUE, 5, 'category'
FROM product_categories pc
WHERE pc.code = 'flight'
  AND NOT EXISTS (
    SELECT 1 FROM category_contracts x
    WHERE x.category_id = pc.id AND x.organization_id IS NULL AND x.code = 'emsal_ucus_bilet_v1'
  );
...
INSERT INTO category_contracts (category_id, organization_id, code, version, is_active, sort_order, contract_scope)
SELECT pc.id, NULL, 'emsal_genel_hizmet_v1', 1, TRUE, 5, 'category'
FROM product_categories pc
WHERE pc.code IN (
...
AND NOT EXISTS (
  SELECT 1 FROM category_contracts x
  WHERE x.category_id = pc.id AND x.organization_id IS NULL AND x.code = 'emsal_genel_hizmet_v1'
);
