...
/** API’de dönem yokken vitrin örneği (tarih aralıkları sabit 2026) */
const DEMO_PERIODS: { from: string; to: string; nightly: number }[] = [
  { from: '2026-01-01', to: '2026-03-18', nightly: 10_000 },
  { from: '2026-03-19', to: '2026-03-29', nightly: 15_000 },
  { from: '2026-03-30', to: '2026-03-31', nightly: 12_500 },
  { from: '2026-04-01', to: '2026-04-12', nightly: 7000 },
  { from: '2026-04-13', to: '2026-04-30', nightly: 6000 },
  { from: '2026-05-01', to: '2026-05-25', nightly: 8000 },
  { from: '2026-05-26', to: '2026-06-30', nightly: 9000 },
  { from: '2026-07-01', to: '2026-08-31', nightly: 11_000 },
]

export function getDemoSeasonalPricingTableRows(
  locale: string,
  currencyCode: string,
  msg: SeasonalPricingCopy,
  options?: SeasonalPricingBuildOptions,
  ...
  return DEMO_PERIODS.map(({ from, to, nightly }) => {
    const periodLabel = `${formatLongDate(from, locale)} ${msg.rangeSep} ${formatLongDate(to, locale)}`
    if (preferDual) {
      const ro = Math.round(nightly * 0.92)
      const mi = Math.round(nightly * 1.08)
      return {
        periodLabel,
        nightlyAmount: nightly,
        weeklyAmount: nightly * 7,
        listingCurrency: code,
        roomOnlyNightly: ro,
        roomOnlyWeekly: ro * 7,
        mealsIncludedNightly: mi,
    ...
    const list = Math.round(nightly * 1.2)
    return {
      periodLabel,
      nightlyAmount: nightly,
      weeklyAmount: nightly * 7,
      listingCurrency: code,
      compareAtNightly: list,
      compareAtWeekly: list * 7,
    }
  })
}
