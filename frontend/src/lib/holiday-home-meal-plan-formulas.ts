...
export const HOLIDAY_MEAL_PLAN_COPY = {
  ...
  pricingMergedLeadMeals: {
    tr: 'Bu tatil evinde ücretlendirme yemek paketine göre yapılır. Tablodaki gecelik tutarlar seçtiğiniz pansiyon seçeneğine uygundur. İşletme, seçtiğiniz pansiyon kapsamında yemek hizmeti sunar.',
    en: 'Pricing for this vacation home depends on the meal package. Nightly amounts in the table match your selected board option. The host provides meals as described for that package.',
    de: 'Die Preise für dieses Ferienhaus richten sich nach der Verpflegungsoption. Die Nachtpreise in der Tabelle entsprechen der gewählten Pension. Verpflegung wird gemäß dieser Option angeboten.',
    ru: 'Цены на этот дом отдыха зависят от питания. Ночные суммы в таблице соответствуют выбранному варианту питания. Заведение предоставляет питание в рамках выбранного варианта.',
    zh: '此度假民宿的价格取决于餐饮方案。表中的每晚金额与您选择的套餐一致。房东按所选套餐提供相应餐饮服务。',
    fr: 'Les tarifs de cette maison de vacances dépendent de la formule repas. Les montants par nuit correspondent à l’option choisie. L’hébergeur assure la restauration prévue par cette formule.',
  },
  pricingMergedLeadBoth: {
    tr: 'Yemeksiz ve yemekli gecelik sütunları döneme göre gösterilir; pansiyon seçenekleri aşağıda özetlenmiştir. İşletme hem yalnız konaklama hem de seçilen pansiyona göre yemek hizmeti sunabilir.',
    en: 'Room-only and with-meals nightly columns are shown by period; meal options are summarized below. The property offers lodging-only stays and board-inclusive options depending on what you choose.',
    de: 'Die Spalten ohne und mit Verpflegung sind nach Zeitraum dargestellt; die Verpflegungsoptionen sind unten zusammengefasst. Es gibt reine Übernachtung sowie Pension je nach gewählter Option.',
    ru: 'Столбцы ночной цены без и с питанием показаны по периодам; варианты питания приведены ниже. Доступны только проживание и варианты с питанием — по вашему выбору.',
    zh: '表格按周期显示不含餐与含餐的每晚价格；餐饮选项摘要见下文。房源同时提供纯住宿与含餐方案，可按需选择。',
    fr: 'Les colonnes par nuit avec ou sans repas sont affichées par période ; les options repas sont résumées ci-dessous. L’établissement propose l’hébergement seul ou avec pension selon votre choix.',
  },
  pricingMergedLeadRoomOnly: {
    tr: 'Gösterilen gecelik tutarlar yalnızca konaklamayı kapsar; yemek dahil değildir. Bu işletmenin yemek hizmeti yoktur.',
    en: 'Nightly rates shown cover lodging only; meals are not included. This property does not provide food service.',
    de: 'Die angezeigten Nachtpreise gelten nur für die Übernachtung; Mahlzeiten sind nicht enthalten. Diese Unterkunft bietet keine Verpflegung oder Gastronomie.',
    ru: 'Указанные ночные тарифы включают только проживание; питание не входит. Объект размещения не предоставляет питание.',
    zh: '所示每晚价格仅含住宿；不含餐。本房源不提供餐饮服务。',
    fr: 'Les tarifs par nuit affichés couvrent uniquement l’hébergement ; les repas ne sont pas inclus. L’établissement ne propose pas de restauration.',
  },
