 ...
 *
 * Backend API (`searchPublicListings`) yanıt vermezse son başarılı snapshot'a düşer.
 *
...
export function mapPublicListingItemToListingBase(
  ...
  const base: TListingBase = {
    ...
    like: false,
    saleOff: isStayRental ? null : discountPct != null ? `%${discountPct}` : null,
    mobileSaleOff: isStayRental && discountPct != null ? `%${discountPct}` : null,
...
export async function fetchFlexibleStayRentalListings(
  ...
  const [apiSearch, holidayPtItems, yachtPtItems] = await Promise.all([
    searchPublicListingsResilient(apiParams, CATALOG_LISTINGS_FETCH_INIT),
...
export async function fetchCategoryListings(
  ...
  const isInteractiveSearch = !!(
    apiLocation ||
    effectiveQuery.checkin?.trim() ||
    effectiveQuery.checkout?.trim() ||
    effectiveQuery.guests?.trim() ||
    effectiveQuery.q?.trim() ||
    effectiveQuery.from?.trim() ||
    effectiveQuery.to?.trim()
  )
  ...
  if (apiSearch.result) {
    ...
    return {
      listings: rows,
      total: vitrinTab ? rows.length : apiSearch.result.total,
      page,
      perPage: apiSearch.result.per_page ?? perPage,
      fromApi: apiSearch.source === 'api',
    }
  }
...
export async function fetchListingsByIds(
  ...
  const apiSearch = await searchPublicListingsResilient(
    {
      categoryCode,
      listingIds: ids,
  ...
  const byId = new Map(
    apiSearch.result.listings.map((it) => [it.id, mapPublicListingItemToListingBase(it, mapOpts)]),
  )
