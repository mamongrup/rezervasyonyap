...
export function getHomepageDefaultModules(m: AppMessages): Omit<PageBuilderModule, 'id'>[] {
  ...
  return [
    ...
    { type: 'featured_by_region', enabled: true, order: 5, config: { heading: h.featuredStay.heading,     subheading: h.featuredStay.subheading,     viewAllHref: '/oteller/all', regions: [] } },
    { type: 'featured_places', enabled: true,  order: 6,   config: { categorySlug: 'oteller',       viewAllHref: '/oteller/all' } },
    { type: 'featured_places', enabled: true,  order: 6.2, config: { categorySlug: 'tatil-evleri',   viewAllHref: '/tatil-evleri/all' } },
    ...
    { type: 'category_grid',   enabled: true,  order: 10, config: { heading: h.categoriesSection.heading, subheading: h.categoriesSection.subheading } },
    { type: 'become_provider', enabled: true,  order: 11, config: { heading: h.becomeProvider.heading,   subheading: h.becomeProvider.subheading,   ctaText: h.becomeProvider.cta, ctaHref: '/ilan-ver', secondaryCtaText: h.becomeProvider.secondaryCta, secondaryCtaHref: '/ilan-ver#nasil-calisir', bgVariant: 'gradient' } },
