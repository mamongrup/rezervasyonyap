...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: category?.name ?? 'Feribot', description: category?.heroSubheading }
}
...
export default async function Page({
  ...
  const { listings, total, page, perPage, fromApi } = await fetchCategoryListings('feribot', query, {
    regionHandle: currentHandle,
  })

  const [filterOptions, heroOverride] = await Promise.all([
    getCarListingFilterOptions(),
    getRegionHeroConfig('feribot', currentHandle ?? ''),
  ])
