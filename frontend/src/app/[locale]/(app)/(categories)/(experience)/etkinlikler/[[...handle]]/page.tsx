...
import { EventTicketCard } from '@/components/cards'
...
export default async function Page({
  ...
  const [filterOptions, heroOverride] = await Promise.all([
    ...
    getRegionHeroConfig('etkinlikler', currentHandle ?? ''),
  ])
  ...
  return (
    <CategoryPageTemplate
      ...
      listingCards={listings.map((l) => (
        <EventTicketCard key={l.id} data={l as any} />
      ))}
      listingCardRenderer={(l) => <EventTicketCard key={l.id} data={l as any} />}
