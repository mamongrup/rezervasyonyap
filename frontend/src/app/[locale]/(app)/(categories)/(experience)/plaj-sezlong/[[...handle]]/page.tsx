...
import { BeachLoungerCard } from '@/components/cards'
...
export async function generateMetadata({
  ...
  return categoryMetadata('plaj-sezlong', locale)
}
...
export default async function Page({
  ...
  const category = getCategoryBySlug('plaj-sezlong')
  ...
  const [filterOptions, heroOverride] = await Promise.all([
    ...
    getRegionHeroConfig('plaj-sezlong', currentHandle ?? ''),
  ])
  ...
  return (
    <CategoryPageTemplate
      ...
      listingCards={listings.map((l) => (
        <BeachLoungerCard key={l.id} data={l as any} />
      ))}
      listingCardRenderer={(l) => <BeachLoungerCard key={l.id} data={l as any} />}
