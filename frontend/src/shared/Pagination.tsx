...
}: React.PropsWithChildren<{ href?: string | null; className?: string; 'aria-label'?: string }>) {
  return (
...
export function PaginationNext({
  ...
  return (
    <span className={clsx(className, 'flex grow basis-0 justify-end')}>
      <Button {...(href === null ? { disabled: true } : { href })} className="rounded-lg" plain aria-label={ariaLabel}>
        {children}
