...
const HeroSearchFormMobile = ({ className, locale: localeProp, open: openProp, onClose: onCloseProp }: HeroSearchFormMobileProps) => {
  ...
  const renderButtonOpenModal = () => (
    <button
      type="button"
      onClick={openModal}
      className="relative box-border flex w-full min-w-0 max-w-full items-center rounded-full border border-neutral-200 px-4 py-2 pe-4 shadow-lg transition hover:shadow-lg sm:pe-11 dark:border-neutral-600 dark:bg-neutral-900"
    >
  ...
  return (
    <div
      className={clsx(
        'min-w-0 w-full max-w-lg',
        !isControlled && 'relative z-10',
        className,
      )}
    >
