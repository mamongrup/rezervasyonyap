...
const Header3: FC<Header3Props> = ({ className, hasBorderBottom = true, initSearchFormTab = 'Stays' }) => {
  ...
  const H = useMemo(() => getMessages(locale).HeroSearchForm, [locale])

  const { locationText, dateText, guestsText } = useMemo(() => {
    if (restPath.startsWith('/turlar') || restPath.startsWith('/aktiviteler') || restPath.startsWith('/kruvaziyer')) {
      return {
        locationText: H['Where do you want to go?'],
        dateText: H['Search dates summary'],
        guestsText: H['Group size'],
      }
    }
    if (
      restPath.startsWith('/arac-kiralama') ||
      restPath.startsWith('/feribot') ||
      restPath.startsWith('/transfer')
    ) {
      return {
        locationText: H['Pick up location'],
        dateText: H['Search dates summary'],
        guestsText: '',
      }
    }
    if (restPath.startsWith('/ucak-bileti')) {
      return {
        locationText: H['Flight search where from'],
        dateText: H['Search dates summary'],
        guestsText: H.Passengers,
      }
    }
    if (
      ...
      return {
        locationText: H['Where do you want to go?'],
        dateText: H['Stay check-in out summary'],
        guestsText: H['Stay map guest hint'],
      }
    }
    return {
      locationText: H.Anywhere,
      dateText: H['Any week'],
      guestsText: H['Add guests'],
    }
  }, [H, restPath])
