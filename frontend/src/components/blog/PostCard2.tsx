    ...
    <div className={`relative flex justify-between gap-x-8 ${className}`}>
      <div className="flex h-full flex-col py-2">
        <h2 className={`block text-base font-semibold nc-card-title`}>
          <Link href={post.detailHref} className="line-clamp-2 capitalize" title={title}>
            {title}
          </Link>
        </h2>
        ...
        <div className="mt-auto hidden sm:block">
          <PostCardMeta author={author} date={date || ''} blogListHref={post.category.href} />
        </div>
      </div>

      <Link href={post.detailHref} className="relative block h-full w-2/5 shrink-0 sm:w-1/3">
        {image?.src && (
