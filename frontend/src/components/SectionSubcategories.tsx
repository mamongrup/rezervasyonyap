...
import { subcategoryVitrinHrefMap } from '@/lib/subcategory-href'
...
export default async function SectionSubcategories({
  parentCategorySlug,
  locale = 'tr',
  subcategories,
  showHeading = true,
  ...
  const hrefById = await subcategoryVitrinHrefMap(locale, items)

  if (variant === 'pill') {
    return <PillVariant items={items} locale={locale} hrefById={hrefById} />
  }
  if (variant === 'card') {
    return (
      <CardVariant items={items} locale={locale} showHeading={showHeading} hrefById={hrefById} />
    )
  }
  return (
    <IconGridVariant items={items} locale={locale} showHeading={showHeading} hrefById={hrefById} />
  )
}
