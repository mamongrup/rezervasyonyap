    ...
    >
      <div className="relative w-full">
        {sliderImages.length > 0 ? (
          <GallerySlider
            uniqueID={`hajj-${String(id)}`}
            ratioClass="aspect-w-4 aspect-h-3"
            galleryImgs={sliderImages}
            href={href}
          />
        ) : (
          <Link href={href} className="relative flex aspect-w-4 aspect-h-3 w-full items-center justify-center overflow-hidden bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-neutral-800 dark:to-neutral-700">
            <span className="text-5xl">🕌</span>
          </Link>
        )}
        <BtnLikeIcon isLiked={like} className="absolute end-3 top-3 z-1" />
        {saleOff ? (
          <span className="absolute start-3 top-3 z-1 rounded-full bg-secondary-500 px-2 py-1 text-xs font-medium text-white">
            <DiscountBadgeLabel text={saleOff} />
          </span>
        ) : null}
        <span className={`absolute bottom-3 start-3 z-1 rounded-full px-2.5 py-1 text-xs font-semibold ${typeColor}`}>
          {typeLabel}
        </span>
      </div>
