...
#   SOCIAL_WORKER_ROTATE=1       — ilk batch'te döngü kuyruğu ekler (varsayılan: kapalı)
#   WORKER_VERBOSE=1             — JSON yanıtını yazdırır
set -euo pipefail
...
while true; do
  ROTATE_PARAM=""
  if [[ "$batch" -eq 1 && "$SOCIAL_WORKER_ROTATE" == "1" ]]; then
    ROTATE_PARAM="&rotate=1"
  fi
