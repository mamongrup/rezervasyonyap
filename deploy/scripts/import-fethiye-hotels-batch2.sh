#!/usr/bin/env bash
# Fethiye kuşağı batch-2 (30 otel: Etstur + TatilBudur kaynak listesi).
# Bookeder + AegeanHotels hasadı → tatilbudur provider import → tema/facet + AI kuyruğu.
#
#   cd /var/www/vhosts/rezervasyonyap.tr/httpdocs
#   git fetch origin main && git reset --hard origin/main
#   chmod +x deploy/scripts/import-fethiye-hotels-batch2.sh
#   SKIP_HARVEST=1 TATILBUDUR_LISTING_STATUS=published ./deploy/scripts/import-fethiye-hotels-batch2.sh
#
# Yeniden hasat: SKIP_HARVEST=0 (varsayılan) veya:
#   node scripts/harvest-ng-hotels.mjs --manifest deploy/data/tatilbudur/fethiye-hotels-batch2.manifest.json \
#     --out deploy/data/tatilbudur/fethiye-hotels-batch2.json
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
MANIFEST="$APP_ROOT/deploy/data/tatilbudur/fethiye-hotels-batch2.manifest.json"
DATA="$APP_ROOT/deploy/data/tatilbudur/fethiye-hotels-batch2.json"
LIMIT="${LIMIT:-0}"

cd "$APP_ROOT"

if [[ "${SKIP_HARVEST:-0}" != "1" ]]; then
  echo "[INFO] Fethiye batch-2 Bookeder/Aegean hasat..."
...
echo "[INFO] Import fethiye-batch2: $count otel"
...
apply_manifest_facets "$MANIFEST" "fethiye_batch2"

echo "[OK] Fethiye hotels batch-2 import tamam ($count otel)."
