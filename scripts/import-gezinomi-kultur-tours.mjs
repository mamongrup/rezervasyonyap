 ...
 *
 *   node scripts/import-gezinomi-kultur-tours.mjs --dry-run --limit 5
 *   node scripts/import-gezinomi-kultur-tours.mjs --category kapadokya --with-images
 *   node scripts/import-gezinomi-kultur-tours.mjs --published
 *
 * Ortam: PG*, GEIZINOMI_DELAY_MS=400, GEIZINOMI_KULTUR_STATUS=draft|published
 */
...
import { fetchAllGezinomiKulturTours } from './lib/gezinomi-kultur-catalog.mjs'
import {
  coverTourImageCandidates,
...
const STATUS =
  PUBLISHED || String(process.env.GEIZINOMI_KULTUR_STATUS || 'draft').toLowerCase() === 'published'
    ? 'published'
    : 'draft'
...
async function main() {
  console.log(`Gezinomi kültür tur import — status=${STATUS}, images=${WITH_IMAGES}, dry-run=${DRY_RUN}`)

  console.log('→ Katalog çekiliyor…')
  let rows = await fetchAllGezinomiKulturTours({ categoryFilter: CATEGORY_FILTER })
  stats.catalog = rows.length
  console.log(`→ ${rows.length} benzersiz tur (productId)`)
  if (LIMIT > 0) rows = rows.slice(0, LIMIT)

  const pgClient = createPgClient()
  if (!DRY_RUN) await pgClient.connect()
  const orgId = process.env.GEIZINOMI_ORG_ID || 'a0000000-0000-4000-8000-000000000001'
  const ctx = DRY_RUN ? { orgId } : await resolveGezinomiTourImportContext(pgClient, orgId)

  for (let i = 0; i < rows.length; i++) {
    const row = rows[i]
    const label = `${row.productId} [${row.tourRegion}] — ${String(row.productName || '').slice(0, 45)}`
    ...
    if (WITH_IMAGES || !DRY_RUN) {
      try {
        ...
        console.warn(`\n    [detail] ${e.message}`)
        galleryUrls = coverTourImageCandidates(row)
      }
      await sleep(DELAY_MS)
    }

    try {
      const result = await upsertGezinomiTourListing(pgClient, ctx, row, {
        status: STATUS,
        detail,
        galleryUrls,
        dryRun: DRY_RUN,
      ...
      if (WITH_IMAGES && !DRY_RUN && galleryUrls.length) {
        ...
        const fallback = coverTourImageCandidates(row)
