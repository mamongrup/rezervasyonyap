/**
 * Villa Bella 1–5 — Birvillas canlı kaynaktan paket üretir.
 * Güncelleme: fiyat, müsaitlik, sistem tema kodları, olanaklar.
 */
import { createPgClient } from './pg-client.mjs'
import {
  amenityLabelsFromBirvillas,
  resolveAmenityAttributeRows,
  systemThemeCodesFromBirvillas,
} from './birvillas-amenity-map.mjs'
import {
  birvillasCalendarDays,
  fetchBirvillasListingPage,
} from './birvillas-listing-page.mjs'

export const MAP_LAT = '36.300408'
export const MAP_LNG = '29.410383'
...
export const FALLBACK_THEMES = ['pool', 'jacuzzi', 'nature', 'conservative', 'family']

export const BELLA_VILLAS = [
  {
    ...
    sourceUrl: 'https://www.birvillas.com/listing/tc97shkNcDvOfEPCKSVs/villa-bella-1-orkide-islamlar',
  ...
  },
  {
    ...
    sourceUrl: 'https://www.birvillas.com/listing/pfosunWEj7iQaf36WVbT/villa-bella-5-kartal-yuvasi-islamlar',
...
export function liveRuleCodes(live) {
...
export function livePools(live, fallback) {
...
/**
 * @param {object} villa
 * @param {{ requireLive?: boolean, resolveAmenities?: boolean }} [opts]
 */
export async function buildBellaVillaPackage(villa, opts = {}) {
  const { requireLive = true, resolveAmenities = true } = opts
  let live = null
  try {
  ...
  const galleryUrls = live?.images?.length
    ...
    : villa.images
  const seasonalPrices = live
    ? (live.dynamicPricing || []).flatMap((row) => (
      row.startDate && row.endDate && Number(row.price) > 0
        ? [{
            from: row.startDate,
            to: row.endDate,
            baseNightly: Number(row.price),
            label: row.name || '',
            minNights: Number(row.minimumStay) > 0 ? Number(row.minimumStay) : undefined,
          }]
        : []
    ))
    : undefined
  ...
  const poolSizeLabel = livePool
    ...
    : villa.poolSize
  const amenityCodes = live?.amenities || []
  const amenities = live ? amenityLabelsFromBirvillas(amenityCodes) : FALLBACK_AMENITIES
  const themeCodes = live ? systemThemeCodesFromBirvillas(live) : FALLBACK_THEMES

  let amenityRows = null
  if (resolveAmenities) {
    try {
      const pg = createPgClient()
      await pg.connect()
      try {
        amenityRows = await resolveAmenityAttributeRows(pg, amenities, amenityCodes)
      } finally {
        await pg.end()
      }
    } catch (error) {
      console.warn(`[WARN] özellik eşlemesi DB olmadan imported_amenity ile devam: ${error.message}`)
      amenityRows = amenities.map((label) => ({
  ...
  return {
    ...
    pkg: {
      ...
      priceNote: live
        ? 'Fiyat ve müsaitlik Birvillas canlı ilan verisinden alınmıştır.'
        : 'Fiyat ve müsaitlik canlı kaynaktan doğrulanmalıdır.',
      meta: {
        ...
        provider_gallery_count: String(live?.images?.length || villa.sourceGalleryCount),
        imported_gallery_count: String(galleryUrls.length),
        media_incomplete: galleryUrls.length < (live?.images?.length || villa.sourceGalleryCount),
      },
    },
    sourceGalleryCount: live?.images?.length || villa.sourceGalleryCount,
  }
}
