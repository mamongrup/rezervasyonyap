/**
 * Travelrobot (KPlus) otel import — checkpoint (kopmaya dayanıklı).
 * Aynı desen: scripts/lib/tatilsepeti-import-checkpoint.mjs
 */
...
const DEFAULT_STATE_PATH = path.join(REPO_ROOT, 'backups', 'travelrobot-hotel-import-state.json')
const DEFAULT_CATALOG_PATH = path.join(REPO_ROOT, 'backups', 'travelrobot-hotel-catalog.json')

export function defaultPaths() {
  return {
    statePath: process.env.TRAVELROBOT_IMPORT_STATE || DEFAULT_STATE_PATH,
    catalogPath: process.env.TRAVELROBOT_IMPORT_CATALOG || DEFAULT_CATALOG_PATH,
  }
}
