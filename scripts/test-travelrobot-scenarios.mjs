...
const TRAVELROBOT_TEST_SCRIPT_VERSION = '2026-06-12-cert-tour-pnr-v36'
      ...
      for (const offset of dateOffsets) {
        ...
        for (const variant of variants) {
          ...
          for (const priceRow of priceRows.slice(0, tourLocked ? 1 : 4)) {
            ...
            const sessionPackageId = tfpSessionId ?? pickTourPaymentSessionId(finalPayload, pricePayload)
            ...
            const sessionBookKeyForPay = pickTourSessionBookKey(
              pickTourPricesSessionRawId(pricePayload),
              finalPricePackageId ?? packageId,
            )
            ...
            for (const pay of paymentAttempts.slice(0, 5)) {
              for (const bodyVariant of bookBodyVariants) {
                for (const paxVariant of tourPaxBase) {
                  ...
                  const sent = lastBookRequest?.request ?? {}
                  try {
            ...
            if (!bookPayload || bookPayload?.HasError) {
              ...
              log(scenarioName, 'BookTour-fail', '/Tour.svc/Rest/Json/BookTour',
                { packageId, resultKeys, attempts: bookAttempts.slice(0, 8), lastBookRequest },
                lastBookResponse ?? lastPriceErr, false)
              if (tourLocked) break attemptLoop
              continue
            }
