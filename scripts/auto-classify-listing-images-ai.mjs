...
async function callAiVision(aiConfig, imagePayload) {
  ...
  if (geminiKey) {
    const candidateModels = activeGeminiModel
      ? [activeGeminiModel]
      : ['gemini-2.0-flash', 'gemini-2.5-flash', 'gemini-1.5-flash-latest', 'gemini-1.5-pro', 'gemini-2.0-flash-exp']
    ...
    for (const model of candidateModels) {
      try {
        ...
        if (res.ok) {
          ...
          // Model bulunamadıysa bir sonrakini dene
          continue
        } else {
