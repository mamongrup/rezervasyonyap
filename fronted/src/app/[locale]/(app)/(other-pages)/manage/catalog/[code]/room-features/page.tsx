...
export default async function ManageCatalogRoomFeaturesPage({ params }: { params: Promise<{ code: string }> }) {
  const { code } = await params
  const c = parseCatalogCategoryCodeParam(code)
  if (!c) return notFound()
  return <CatalogModuleSectionClient categoryCode={c} section="room-features" />
}
