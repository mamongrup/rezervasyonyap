    ...
    <div className="listingSection__wrap">
      <div className="flex items-center gap-x-4">
        ...
        <div>
          ...
          <div className="mt-1.5 flex items-center text-sm text-neutral-500 dark:text-neutral-400">
            ...
            <span>{interpolate(h.listingsCount, { count: String(listingsCount) })}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-y-2.5 text-neutral-700 dark:text-neutral-300">
        <div className="flex items-center gap-x-3">
          ...
          <span>{interpolate(h.memberSince, { date: joinedDate })}</span>
        </div>
        <div className="flex items-center gap-x-3">
          <Comment01Icon className="h-6 w-6" />
          <span>{interpolate(h.responseRate, { rate: String(responseRate) })}</span>
        </div>
        <div className="flex items-center gap-x-3">
          <Timer02Icon className="h-6 w-6" />
          <span>{interpolate(h.responseTime, { time: responseTime })}</span>
        </div>
      </div>
