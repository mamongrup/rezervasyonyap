...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: category?.name ?? 'Aktiviteler', description: category?.heroSubheading }
}
...
export default async function Page({
  ...
  const { listings, total, page, perPage, fromApi } = await fetchCategoryListings('aktiviteler', query, {
    regionHandle: currentHandle,
  })

  const [filterOptions, heroOverride] = await Promise.all([
    getExperienceListingFilterOptions(),
    getRegionHeroConfig('aktiviteler', currentHandle ?? ''),
  ])
