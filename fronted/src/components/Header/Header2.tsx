...
import { getNavigationLocalized, resolveHeaderCurrencies } from '@/data/navigation'
...
const Header2: FC<Props> = async ({ hasBorder = true, className }) => {
  const navigationMenu = await getNavigationLocalized('tr')
  const featuredCategory = await resolveMegaMenuFeatured('tr')
      ...
      >
        <div className="container flex h-16 justify-between">
          ...
          <div className="flex flex-1 items-center justify-end gap-x-2.5 sm:gap-x-6">
            <GlobalSearch />
            <CurrLangDropdown currencies={currencies} locale="tr" className="hidden md:block" />
            <NotifyDropdown />
            <AvatarDropdown />
          </div>
        </div>
